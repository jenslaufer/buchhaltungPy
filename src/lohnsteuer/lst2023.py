...
class Lohnsteuer:
    ...
    def MPARA(self):
        ...
        if self.KRV < 2:
            if self.KRV == 0:
                self.BBGRV = BigDecimal(87600)
            else:
                self.BBGRV = BigDecimal(85200)
            self.RVSATZAN = BigDecimal.valueOf(0.093)
            self.TBSVORV = BigDecimal.valueOf(1.0)
        self.BBGKVPV = BigDecimal(59850)
        self.KVSATZAN = self.KVZ.divide(Lohnsteuer.ZAHL2).divide(Lohnsteuer.ZAHL100).add(BigDecimal.valueOf(0.07))
        self.KVSATZAG = BigDecimal.valueOf(0.008).add(BigDecimal.valueOf(0.07))
        if self.LZZ == 1:
            if self.PVS == 1:
                self.PVSATZAN = BigDecimal.valueOf(0.021125)
                self.PVSATZAG = BigDecimal.valueOf(0.011125)
            else:
                self.PVSATZAN = BigDecimal.valueOf(0.016125)
                self.PVSATZAG = BigDecimal.valueOf(0.016125)
            if self.PVZ == 1:
                self.PVSATZAN = self.PVSATZAN.add(BigDecimal.valueOf(0.00475))
        else:
            if self.PVS == 1:
                self.PVSATZAN = BigDecimal.valueOf(0.022)
                self.PVSATZAG = BigDecimal.valueOf(0.012)
            else:
                self.PVSATZAN = BigDecimal.valueOf(0.017)
                self.PVSATZAG = BigDecimal.valueOf(0.017)
            if self.PVZ == 1:
                self.PVSATZAN = self.PVSATZAN.add(BigDecimal.valueOf(0.006))
        self.W1STKL5 = BigDecimal(12485)
        self.W2STKL5 = BigDecimal(31404)
        self.W3STKL5 = BigDecimal(222260)
        self.GFB = BigDecimal(10908)
        self.SOLZFREI = BigDecimal(17543)
    ...
    def MRE4(self):
        ...
        if self.ZVBEZJ.compareTo(BigDecimal.ZERO) == 0:
            ...
            if self.LZZ == 1:
                ...
                self.HFVB = Lohnsteuer.TAB2[self.J].divide(Lohnsteuer.ZAHL12).multiply(BigDecimal.valueOf(self.ZMVB))
    ...
    def MRE4ABZ(self):
        ...
        if self.KENNVMT == 2:
            self.ZRE4VP = self.ZRE4VP.subtract(self.ENTSCH.divide(Lohnsteuer.ZAHL100)).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def MZTABFB(self):
        ...
        if self.STKL == 1:
            ...
            self.KFB = self.ZKF.multiply(BigDecimal.valueOf(8952)).setScale(0, BigDecimal.ROUND_DOWN)
        else:
            if self.STKL == 2:
                ...
                if self.STKL == 3:
                    ...
                    if self.STKL == 4:
                        self.SAP = BigDecimal.valueOf(36)
                        self.KFB = self.ZKF.multiply(BigDecimal.valueOf(4476)).setScale(0, BigDecimal.ROUND_DOWN)
                    else:
                        if self.STKL == 5:
                            self.SAP = BigDecimal.valueOf(36)
    ...
    def MLSTJAHR(self):
        ...
        if self.KENNVMT != 1:
            ...
            self.ZVE = self.ZRE4.subtract(self.ZTABFB).subtract(self.VSP).subtract(self.VMT.divide(Lohnsteuer.ZAHL100)).subtract(self.VKAPA.divide(Lohnsteuer.ZAHL100)).setScale(2, BigDecimal.ROUND_DOWN)
            if self.ZVE.compareTo(BigDecimal.ZERO) == -1:
                self.ZVE = self.ZVE.add(self.VMT.divide(Lohnsteuer.ZAHL100)).add(self.VKAPA.divide(Lohnsteuer.ZAHL100)).divide(Lohnsteuer.ZAHL5).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.multiply(Lohnsteuer.ZAHL5).setScale(0, BigDecimal.ROUND_DOWN)
            else:
                self.UPMLST()
                self.STOVMT = self.ST
                self.ZVE = self.ZVE.add(self.VMT.add(self.VKAPA).divide(Lohnsteuer.ZAHL500)).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.subtract(self.STOVMT).multiply(Lohnsteuer.ZAHL5).add(self.STOVMT).setScale(0, BigDecimal.ROUND_DOWN)
    ...
    def UPVKV(self):
        ...
        if self.PKV > 0:
            if self.VSP2.compareTo(self.VSP3) == 1:
                self.VKV = self.VSP2.multiply(Lohnsteuer.ZAHL100)
            else:
                self.VKV = self.VSP3.multiply(Lohnsteuer.ZAHL100)
        else:
    ...
    def MVSP(self):
        ...
        if self.PKV > 0:
            if self.STKL == 6:
                ...
                self.VSP3 = self.PKPV.multiply(Lohnsteuer.ZAHL12).divide(Lohnsteuer.ZAHL100)
                if self.PKV == 2:
    ...
    def MVMT(self):
        ...
        if self.VMT.add(self.VKAPA).compareTo(BigDecimal.ZERO) == 1:
            ...
            self.ZRE4J = self.JRE4.add(self.SONSTB).add(self.VMT).add(self.VKAPA).divide(Lohnsteuer.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.JVBEZ.add(self.VBS).add(self.VKAPA).divide(Lohnsteuer.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.KENNVMT = 2
            self.MRE4SONST()
            self.MLSTJAHR()
            self.LST3 = self.ST.multiply(Lohnsteuer.ZAHL100)
            self.MRE4ABZ()
            self.ZRE4VP = self.ZRE4VP.subtract(self.JRE4ENT.divide(Lohnsteuer.ZAHL100)).subtract(self.SONSTENT.divide(Lohnsteuer.ZAHL100))
            self.KENNVMT = 1
            self.MLSTJAHR()
            self.LST2 = self.ST.multiply(Lohnsteuer.ZAHL100)
            ...
            if self.STV.compareTo(BigDecimal.ZERO) == -1:
                ...
                self.STV = self.STV.multiply(BigDecimal.valueOf(self.f)).divide(Lohnsteuer.ZAHL100, 0, BigDecimal.ROUND_DOWN).multiply(Lohnsteuer.ZAHL100)
            self.SOLZVBMG = self.STV.divide(Lohnsteuer.ZAHL100, 0, BigDecimal.ROUND_DOWN).add(self.JBMG)
            if self.SOLZVBMG.compareTo(self.SOLZFREI) == 1:
                self.SOLZV = self.STV.multiply(BigDecimal.valueOf(5.5)).divide(Lohnsteuer.ZAHL100, 0, BigDecimal.ROUND_DOWN)
            else:
    ...
    def UPTAB23(self):
        ...
        if self.X.compareTo(self.GFB.add(Lohnsteuer.ZAHL1)) == -1:
            ...
            if self.X.compareTo(BigDecimal.valueOf(16000)) == -1:
                self.Y = self.X.subtract(self.GFB).divide(Lohnsteuer.ZAHL10000, 6, BigDecimal.ROUND_DOWN)
                self.RW = self.Y.multiply(BigDecimal.valueOf(979.18))
                self.RW = self.RW.add(BigDecimal.valueOf(1400))
                self.ST = self.RW.multiply(self.Y).setScale(0, BigDecimal.ROUND_DOWN)
            else:
                if self.X.compareTo(BigDecimal.valueOf(62810)) == -1:
                    self.Y = self.X.subtract(BigDecimal.valueOf(15999)).divide(Lohnsteuer.ZAHL10000, 6, BigDecimal.ROUND_DOWN)
                    self.RW = self.Y.multiply(BigDecimal.valueOf(192.59))
                    self.RW = self.RW.add(BigDecimal.valueOf(2397))
                    self.RW = self.RW.multiply(self.Y)
                    self.ST = self.RW.add(BigDecimal.valueOf(966.53)).setScale(0, BigDecimal.ROUND_DOWN)
                else:
                    if self.X.compareTo(BigDecimal.valueOf(277826)) == -1:
                        self.ST = self.X.multiply(BigDecimal.valueOf(0.42)).subtract(BigDecimal.valueOf(9972.98)).setScale(0, BigDecimal.ROUND_DOWN)
                    else:
                        self.ST = self.X.multiply(BigDecimal.valueOf(0.45)).subtract(BigDecimal.valueOf(18307.73)).setScale(0, BigDecimal.ROUND_DOWN)

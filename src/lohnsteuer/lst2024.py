...
class Lohnsteuer:
    ...
    def MPARA(self):
        ...
        if self.KRV < 2:
            if self.KRV == 0:
                self.BBGRV = BigDecimal(90600)
            else:
                self.BBGRV = BigDecimal(89400)
            self.RVSATZAN = BigDecimal.valueOf(0.093)
        self.BBGKVPV = BigDecimal(62100)
        self.KVSATZAN = self.KVZ.divide(Lohnsteuer.ZAHL2).divide(Lohnsteuer.ZAHL100).add(BigDecimal.valueOf(0.07))
        self.KVSATZAG = BigDecimal.valueOf(0.0085).add(BigDecimal.valueOf(0.07))
        if self.PVS == 1:
        ...
        self.W1STKL5 = BigDecimal(13279)
        self.W2STKL5 = BigDecimal(33380)
        self.W3STKL5 = BigDecimal(222260)
        self.GFB = BigDecimal(11604)
        self.SOLZFREI = BigDecimal(18130)
    ...
    def MZTABFB(self):
        ...
        if self.STKL == 1:
            ...
            self.KFB = self.ZKF.multiply(BigDecimal.valueOf(9312)).setScale(0, BigDecimal.ROUND_DOWN)
        else:
            if self.STKL == 2:
                ...
                if self.STKL == 3:
                    ...
                    if self.STKL == 4:
                        self.SAP = BigDecimal.valueOf(36)
                        self.KFB = self.ZKF.multiply(BigDecimal.valueOf(4656)).setScale(0, BigDecimal.ROUND_DOWN)
                    else:
                        if self.STKL == 5:
                            self.SAP = BigDecimal.valueOf(36)
    ...
    def UPTAB24(self):
        ...
        if self.X.compareTo(self.GFB.add(Lohnsteuer.ZAHL1)) == -1:
            ...
            if self.X.compareTo(BigDecimal.valueOf(17006)) == -1:
                self.Y = self.X.subtract(self.GFB).divide(Lohnsteuer.ZAHL10000, 6, BigDecimal.ROUND_DOWN)
                self.RW = self.Y.multiply(BigDecimal.valueOf(922.98))
                self.RW = self.RW.add(BigDecimal.valueOf(1400))
                self.ST = self.RW.multiply(self.Y).setScale(0, BigDecimal.ROUND_DOWN)
            else:
                if self.X.compareTo(BigDecimal.valueOf(66761)) == -1:
                    self.Y = self.X.subtract(BigDecimal.valueOf(17005)).divide(Lohnsteuer.ZAHL10000, 6, BigDecimal.ROUND_DOWN)
                    self.RW = self.Y.multiply(BigDecimal.valueOf(181.19))
                    self.RW = self.RW.add(BigDecimal.valueOf(2397))
                    self.RW = self.RW.multiply(self.Y)
                    self.ST = self.RW.add(BigDecimal.valueOf(1025.38)).setScale(0, BigDecimal.ROUND_DOWN)
                else:
                    if self.X.compareTo(BigDecimal.valueOf(277826)) == -1:
                        self.ST = self.X.multiply(BigDecimal.valueOf(0.42)).subtract(BigDecimal.valueOf(10602.13)).setScale(0, BigDecimal.ROUND_DOWN)
                    else:
                        self.ST = self.X.multiply(BigDecimal.valueOf(0.45)).subtract(BigDecimal.valueOf(18936.88)).setScale(0, BigDecimal.ROUND_DOWN)
